...
The VSCP server (the VSCP daemon) is no longer maintained in a special version for Windows. However it runs very well in the bash mode of Windows. Follow the instructions [here](https://www.howtogeek.com/249966/how-to-install-and-use-the-linux-bash-shell-on-windows-10/) to enable bash and then follow the [Linux/Unix installation](http://www.vscp.org/docs/vscpd/doku.php?id=setting_up_the_system_on_unix) process.
...
Here is a short description of what you find in this folder.
 ...
 | **mkpasswd.exe**   | With this utility you can calculate the password hash for users. The hash is constructed by calculating an md5 checksum over **"username:auth-domain:password"**. The auth-domain is set in the vscpd.conf configuration file and is [described here](./configuring_the_vscp_daemon.md#webserver). | 
 | **uninstall.exe**  | You can uninstall VSCP & Friends with this command | 
 | **drivers\level1** | Contains level I drivers that you can use with the VSCP daemon. They are described [here](./level_i_drivers.md).| 
 | **drivers\level2** | Contains level II drivers that you can use with the VSCP daemon. They are described her [here](./level_ii_drivers.md). | 
 | **include**        | This folder contains include files for program development using the [helper lib](https://www.vscp.org/docs/vscphelper/doku.php?id=start). | 
 | **lib**            | This folder contains the helper dll and include lib used for program development using the [helper lib](https://www.vscp.org/docs/vscphelper/doku.php?id=start).  | 
...
after an installation. Here is a short description of what you can expect to find in this folder.
 ...
 | File/Folder       | Description     | 
 | -----------       | -----------     | 
 | **actions**       | This folder contains action scripts run from the [decision matrix](./decision_matrix.md) | 
...
To take a newly installed VSCP & Friends system on a test ride [go to this page](./new_system_install_test_ride.md).
